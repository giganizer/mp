...
e3b_line_edges = findedges(e3b_oneline, 2, 25)
e3b_rect_edges = findedges(e3b_rectangle, 2, 25)
e3b_hough_2 = hough_find_lines(e3b_oneline, (180, 180), None)
e3b_hough_3 = hough_find_lines(e3b_rectangle, (180, 180), None)
    ...
    for y in range(0, thrsh_acc.shape[0]):
        for x in range(0, thrsh_acc.shape[1]):
            if thrsh_acc[y][x] == 1: 
                theta = theta_space[x]
                rho = rho_space[y]
                ret_thetas.append(theta)
                ret_rhos.append(rho)
    return (ret_rhos, ret_thetas)

# %%
e3d_synt_hgh_nms = nonmaxima_suppression_box(e3b_hough_1)
e3d_line_hgh_nms = nonmaxima_suppression_box(e3b_hough_2)
e3d_rect_hgh_nms = nonmaxima_suppression_box(e3b_hough_3)

e3d_pairs_synt = pair_extraction(e3d_synt_hgh_nms, 1, (180, 180))
e3d_pairs_line = pair_extraction(e3d_line_hgh_nms, 1200, (180, 180))
e3d_pairs_rect = pair_extraction(e3d_rect_hgh_nms, 60, (180, 180))
