...
def ransac(matches, x1, y1, x2, y2, thresh, percent):
    ...
    for it in range(2000):
        ...
        for m1, m2 in matches:
            ...
            point1 = np.array([xp1, yp1, 1]).reshape(1, 3)
            point2 = np.array([xp2, yp2, 1]).reshape(1, 3)
            point1 = point1 @ h
        ...
        errorfinal = np.sum(errors) if len(errors)>0 else np.inf
...
# %%
b3inlset, b3h = ransac(b3matches, b3xa, b3yb, b3xb, b3yb, 4000, 0.75)
